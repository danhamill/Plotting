...
#Buffered points
buff_shp = r"C:\workspace\Reach_4a\Multibeam\mb_sed_class\output\shapefiles\may2014_10m_buff.shp"

#Resampled sed5Class
sed_pts = r"C:\workspace\Reach_4a\Multibeam\mb_sed_class\may2014_mb6086r_sedclass\xy_sed5class_10m.csv"

#WGS shape files
buf_geo = r"C:\workspace\Reach_4a\Multibeam\mb_sed_class\output\shapefiles\may2014_10m_buff_geo.shp"
...
ax.set_title('R02028: 10 meter grid')
...
m.readshapefile(r"C:\workspace\Reach_4a\Multibeam\mb_sed_class\output\shapefiles\may2014_10m_buff_geo","layer",drawbounds = False)
#sand, sand/gravel, gravel, sand/rock, rock
s_patch, sg_patch, g_patch, sr_patch, r_patch, = [],[],[],[],[]
bound = max(stat['count'] for stat in stats_28)
...
ax.set_title('R02031 10 meter grid')
m = Basemap(projection='merc', 
            epsg=cs2cs_args.split(':')[1], 
            llcrnrlon=np.nanmin(r28_lon)-0.0004, 
            llcrnrlat=np.nanmin(r28_lat)-0.0004,
...
bound = max(stat['count'] for stat in stats_31)
for info, shape in zip(m.layer_info, m.layer):
...
plt.savefig(r"C:\workspace\Reach_4a\Multibeam\mb_sed_class\output\mb_sed_class_ground_truth_10m_full_polygon.png",dpi=1000)
