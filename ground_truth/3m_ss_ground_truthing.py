...
#Append count_28 to shapefile
ds = ogr.Open(buf_geo,update=True)
lyr = ds.GetLayer()
lyr.CreateField(ogr.FieldDefn('count_28', ogr.OFTReal))
n=0
for feature in lyr:
    count = float(count_28['count'][n])
    feature.SetField('count_28', count)
    n += 1
    lyr.SetFeature(feature)
del ds, lyr, n, count, count_28
...
n=0
for feature in lyr:
    count = float(count_31['count'][n])
    feature.SetField('count_31', count)
    n += 1
    lyr.SetFeature(feature)
del ds, lyr, n, count, count_31
...
m = Basemap(projection='merc', 
            epsg=cs2cs_args.split(':')[1], 
            llcrnrlon=np.nanmin(r31_lon)-0.0009, 
            llcrnrlat=np.nanmin(r31_lat)-0.0006,
            urcrnrlon=np.nanmax(r31_lon)+0.0009, 
            urcrnrlat=np.nanmax(r31_lat)+0.0006)
m.wmsimage(server='http://grandcanyon.usgs.gov/arcgis/services/Imagery/ColoradoRiverImageryExplorer/MapServer/WmsServer?', layers=['3'], xpixels=1000)
...
plt.savefig(r"C:\workspace\Reach_4a\Multibeam\mb_sed_class\output\mb_sed_class_ground_truth_3m.png",dpi=1000)
