...
if  __name__ == '__main__':
    ...
    #things to change
    files = glob(r"C:\workspace\Reach_4a\2014_05\R01356\R01356x_y_ss*")
    scan = 'R01356'
    date = '2014_05'
    
    #read all files into csv    
    for thing in files:
        df = pd.read_csv(thing, sep=' ', header = None, names = ['easting', 'northing','sidescan_intensity'])
        if thing == files[0]:
            out_df = df
        else:
    ...
    driverName= 'GTiff'    
    proj = data['proj']
    ss_outFile = r"C:\workspace\Merged_SS\raster"+ os.sep +'ss_' + date + '_' + scan + '_raster.tif'
