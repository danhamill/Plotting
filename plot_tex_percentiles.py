...
def get_bounds(df):
    lbound = df.describe().iloc[5][0]
    ubound = df.describe().iloc[6][0]
...
lbound, ubound = get_bounds(df_10)
tex_data_10[tex_data_10<lbound]= np.nan
tex_data_10[tex_data_10>ubound]= np.nan
im2 = m.contourf(x, y, tex_data_10.T, alpha=0.4, cmap='YlOrRd', levels=tex_levels)
...
lbound, ubound = get_bounds(df_20)
tex_data_20[tex_data_20<lbound]= np.nan
tex_data_20[tex_data_20>ubound]= np.nan
...
lbound, ubound = get_bounds(df_40)
tex_data_40[tex_data_40<lbound]= np.nan
tex_data_40[tex_data_40>ubound]= np.nan
...
lbound, ubound = get_bounds(df_80)
...
lbound, ubound = get_bounds(df_160)
...
cbr = plt.colorbar(im, cax=cax)
cbr2 = plt.colorbar(im2,cax=cax2)
plt.suptitle('Texture Lengthscale 50th - 75th Percentile', fontsize=12)
plt.savefig(r"C:\workspace\Merged_SS\window_analysis\output\tex_percentile_50th-75th.png",dpi=1000)
#plt.show()
